...
def create_followup_mission(customer, report_text, original_topic):
    try:
        ...
        ภารกิจ: สร้างงานใหม่ โดยเลือกวันที่ตามกฎเท่านั้น (ห้ามเดา):
        
        🔥 กฎการเลือกวัน (Priority):
        1. **ต้องเจอคำระบุเวลาชัดเจนเท่านั้น** ถึงจะใช้วันที่นั้นได้:
           - คำที่ยอมรับ: "พรุ่งนี้", "วันจันทร์หน้า", "วันที่ 5", "สิ้นเดือน"
           
        2. **คำต้องห้าม (Ambiguous Words):**
           - คำว่า "เดี๋ยว", "ก่อน", "รอดู", "ช่วงนี้", "เร็วๆนี้" -> **ห้ามตีความเป็นวันพรุ่งนี้เด็ดขาด!**
           - ถ้าเจอคำพวกนี้แต่ไม่มีวันที่ -> ให้ถือว่า **"ไม่ระบุวัน"**
           
        3. **Default Action (ไม่ระบุวัน):**
           - ให้ใช้ค่า **"{next_month_str}"** (เดือนหน้า) เท่านั้น
