...
# ==========================================
# 3.3 สร้างงานติดตามผลอัตโนมัติ (Fixed Logic: Date Priority)
# ==========================================
def create_followup_mission(customer, report_text, manual_status=""):
    try:
        ...
        prompt = f"""
        Role: ระบบ CRM อัจฉริยะ
        Date Today: {today}
        Report: "{report_text}"
        Status: "{manual_status}"
        
        ภารกิจ: สร้าง "งานติดตามผล" (Next Mission) ลงปฏิทิน
        
        🔥 กฎเหล็กสูงสุด (Ultimate Rules):
        
        1. **Rule #0 (สำคัญที่สุด):** ถ้าในรายงานมี "วันที่", "เดือน", หรือ "ช่วงเวลา" (เช่น 5 ธันวา, สิ้นเดือน, ปีหน้า)
           - **ต้องสร้างงานทันที** (`create`: true) ไม่ว่า Status จะเป็นอะไรก็ตาม
           - ถือว่าเป็นวันนัดหมายสำคัญที่ห้ามพลาด
           - **Topic:** ต้องระบุวันที่ในวงเล็บเสมอ เช่น "ติดตามออเดอร์ (นัด 5 ธ.ค.)"
           
        2. **Rule #1:** ถ้าไม่มีวันที่ระบุ แต่ Status คือ "รอสรุป/ติดตามต่อ"
           - ให้สร้างงานติดตามในอีก 2 สัปดาห์
           
        3. **Rule #2:** ถ้าไม่มีวันที่ และ Status คือ "จบงาน/ขายได้" หรือ "ปฏิเสธ"
           - ให้สร้างงาน "Monthly Visit" (เยี่ยมประจำเดือน) ในเดือนถัดไป
        
        Output JSON Format:
        {{
            "create": true,
            "topic": "หัวข้อ (ใส่วันที่ด้วยถ้ามี)",
            "desc": "รายละเอียด (ดึงจากรายงาน)",
            "status": "pending"
        }}
        """
        ...
        completion = client.chat.completions.create(
            model="llama-3.1-8b-instant",
            messages=[{"role": "user", "content": prompt}],
            temperature=0.0, # ใช้ 0.0 เพื่อความแม่นยำสูงสุด (ไม่ใช้จินตนาการ)
        ...
        return {
            "create": True, 
            "topic": "Follow up (Auto)", 
            "desc": f"ติดตามงานต่อ: {report_text}", 
            "status": "pending"
        }
# ==========================================
# 4. LOAD DATA
