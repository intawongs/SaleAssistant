...
st.set_page_config(page_title="RC Sales AI (Context Aware)", layout="wide", page_icon="🧠")
...
# ==========================================
# 3. AI LOGIC (Groq) - [Updated Summary]
# ==========================================
...
def summarize_voice_report(raw_text, customer_name, mission_df):
    try:
        ...
        if not mission_df.empty:
            ...
            tasks_text = "ไม่มีโจทย์พิเศษ (รายงานทั่วไป)"

        prompt = f"""
        Task: สรุปรายงานการขาย โดยการ "จับคู่โจทย์ กับ คำตอบ"
        Customer: {customer_name}
        
        --- ข้อมูล ---
        📋 รายการโจทย์ (Questions):
        {tasks_text}
        
        🗣️ เสียงรายงานจากเซลล์ (Answers):
        "{raw_text}"
        ...
        completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile", # ใช้ตัว 70B เพื่อความฉลาดในการจับคู่
...
# --- SALES REP ---
    ...
    if df_today.empty:
        ...
        st.divider()
        st.write("🎙️ **รายงานผล (AI จะจับคู่คำตอบให้):**")
                ...
                if audio['bytes'] != st.session_state.last_audio:
                    ...
                    with st.spinner("กำลังจับคู่คำตอบ..."):
                        ...
                        if raw_text:
                            ...
                            # ส่ง df_today (โจทย์) ไปให้ AI ดูด้วย
                            summary = summarize_voice_report(raw_text, target_cust, df_today)
                            st.session_state.report_text_buffer = summary
                            st.rerun()
            
            final_report = st.text_area("📝 สรุปจาก AI:", value=st.session_state.report_text_buffer, height=200)
