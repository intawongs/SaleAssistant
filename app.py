...
# ==========================================
# 3.3 [FINAL LOGIC] สร้างงานต่อ (Auto-Date Mapping)
# ==========================================
def create_followup_mission(customer, report_text, manual_status=""):
    try:
        ...
        Date Today: {today}
        Input: "{report_text}" (Status: {manual_status})
        
        ภารกิจ: ปิดจ็อบงานเก่า และสร้าง "งานใหม่" เสมอ (`create`: true)
        
        🔥 กฎการแปลงวันที่ (Date Logic Conversion):
        ให้สแกนหาคำระบุเวลา แล้วแปลงเป็น "วันนัดหมาย" ในหัวข้อ (Topic) ดังนี้:
        
        1. **กรณีเจอวันที่เป๊ะๆ** (เช่น 5 ธ.ค., วันที่ 20):
           -> Topic: "ติดตามงาน ([วันที่ที่เจอ])"
           
        2. **กรณีเจอชื่อเดือน** (เช่น มกราคม, เดือนหน้า):
           -> ให้แปลงเป็น "วันที่ 1" ของเดือนนั้น
           -> Topic: "ติดตามงาน (1 [ชื่อเดือน])" 
           
        3. **กรณีเจอไตรมาส** (เช่น ไตรมาส 1, Q1, ต้นปี):
           -> ให้แปลงเป็น "วันที่ 1" ของเดือนแรกในไตรมาสนั้น
           -> Q1 = 1 ม.ค., Q2 = 1 เม.ย., Q3 = 1 ก.ค., Q4 = 1 ต.ค.
           -> Topic: "ติดตามงาน (1 [เดือนแรกของไตรมาส])"
           
        4. **กรณีไม่เจอเวลาเลย**:
           -> Topic: "Monthly Visit (รอบเดือนถัดไป)"
        
        Output JSON:
        {{
            "create": true,
            "topic": "หัวข้อตามกฎด้านบน",
            "desc": "รายละเอียดสั้นๆ จากรายงานเดิม: {report_text}",
            "status": "pending"
        }}
        """
        ...
        return {
            ...
            "desc": f"ติดตามต่อเนื่อง: {report_text}", 
            "status": "pending"
        }
# ==========================================
# 4. LOAD DATA
