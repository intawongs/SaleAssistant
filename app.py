...
# 3.3 Auto-Followup (จับทุกเงื่อนไขเวลา: วัน/เดือน/ไตรมาส)
# ==========================================
def create_followup_mission(customer, report_text):
    try:
        ...
        prompt = f"""
        Role: ระบบ CRM อัจฉริยะ ผู้เชี่ยวชาญด้านการนัดหมาย
        Date Today: {today}
        Report Text: "{report_text}"
        
        ภารกิจ: วิเคราะห์ข้อความเพื่อสร้าง "งานติดตามผล" (Next Mission)
        
        🔥 กฎการจับเวลา (Time Detection Rules):
        ให้ค้นหาคำระบุเวลาในข้อความ ไม่ว่าจะเป็นรูปแบบไหนก็ตาม:
        1. **ระบุวันที่:** "วันที่ 5", "กลางเดือน", "สิ้นเดือน"
        2. **ระบุเดือน:** "มกราคม", "เดือนหน้า", "อีก 2 เดือน"
        3. **ระบุไตรมาส:** "ไตรมาสหน้า", "Q1", "Q3", "ครึ่งปีแรก", "ปลายปี"
        
        --- เงื่อนไขการสร้างงาน ---
        
        CASE A: ถ้าเจอคำระบุเวลา (ตามกฎด้านบน)
        - **สร้างงานทันที** (`create`: true)
        - **Topic:** "ติดตามงาน: [เรื่องที่คุย] (นัดหมาย: [เวลาที่เจอ])"
        - **Desc:** "ระบบจับเวลาได้จากรายงาน: {report_text}"
        
        CASE B: ถ้าไม่เจอเวลาเลย (หรือลูกค้าปฏิเสธ/ยังไม่สนใจ)
        - **สร้างงานเยี่ยมประจำเดือน**
        - **Topic:** "Monthly Visit"
        - **Desc:** "เข้าเยี่ยมเพื่อรักษาความสัมพันธ์ประจำเดือน"
